...
export default function RepoLoadedState() {
  const { parsedRepo, analysis, loadedAt, reset } = useAppState();
    ...
    <Paper variant="outlined" sx={{ p: 3, width: "100%" }}>
      ...
      {analysis && (
        <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1, mt: 1.5 }}>
          <Chip
            icon={<PeopleIcon />}
            label={`${analysis.contributors?.length ?? 0} contributors`}
            size="small"
            variant="outlined"
          />
          <Chip
            icon={<CommitIcon />}
            label={`${analysis.commits?.length ?? 0} commits`}
            size="small"
            variant="outlined"
          />
          <Chip
            icon={<MergeIcon />}
            label={`${analysis.pullRequests?.length ?? 0} PRs`}
            size="small"
            variant="outlined"
          />
          <Chip
            icon={<FolderIcon />}
            label={`${analysis.repositoryContents?.length ?? 0} root items`}
            size="small"
            variant="outlined"
          />
        </Box>
